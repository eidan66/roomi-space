...
export class AdvancedRoomCalculator {
    ...
    static calculateRoomMetrics(walls: Wall[]): RoomMetrics {
        ...
        const averageWallLength = wallLengths.reduce((sum, len) => sum + len, 0) / wallLengths.length;
        ...
        const averageAngle = interiorAngles.reduce((sum, angle) => sum + angle, 0) / interiorAngles.length;

        // Validate angles
        interiorAngles.forEach((angle, index) => {
            const angleDegrees = (angle * 180) / Math.PI;
            if (angleDegrees < this.MIN_ANGLE || angleDegrees > this.MAX_ANGLE) {
                validationErrors.push(`Corner ${index + 1} has extreme angle (${angleDegrees.toFixed(1)}°)`);
            }
        });
    ...
    private static getOrderedVertices(walls: Wall[]): Point[] {
        ...
        while (vertices.length < walls.length) {
            ...
            if (!nextPoint) break; // No more unvisited neighbors
    ...
    private static calculatePerimeter(walls: Wall[]): number {
        return walls.reduce((sum, wall) => sum + this.calculateDistance(wall.start, wall.end), 0);
    }
    ...
    private static calculateRectangularity(vertices: Point[]): number {
        ...
        const ratio1 = Math.min(sideLengths[0], sideLengths[2]) / Math.max(sideLengths[0], sideLengths[2]);
        const ratio2 = Math.min(sideLengths[1], sideLengths[3]) / Math.max(sideLengths[1], sideLengths[3]);
    ...
    private static calculateConvexity(vertices: Point[]): number {
        ...
        for (let i = 0; i < vertices.length; i++) {
            ...
            if (Math.abs(crossProduct) > this.PRECISION) {
                if (lastCrossProduct !== 0 && Math.sign(crossProduct) !== Math.sign(lastCrossProduct)) {
                    signChanges++;
                }
