...
export class RoomGeometry {
  ...
  static buildConnectionGraph(walls: Wall[]): Map<string, { point: { x: number; z: number }; connections: { x: number; z: number }[] }> {
    const graph = new Map();
  ...
  static isValidRoom(walls: Wall[]): boolean {
    ...
    for (const [_, data] of graph.entries()) {
      if (data.connections.length !== 2) return false;
    }
  ...
  static getOrderedVertices(walls: Wall[]): { x: number; z: number }[] {
    ...
    for (const [_, data] of graph.entries()) {
      if (data.connections.length === 2) {
        if (data.point.x < minX || (Math.abs(data.point.x - minX) < this.PRECISION && data.point.z < minZ)) {
          minX = data.point.x;
          minZ = data.point.z;
    ...
    while (currentPoint && orderedVertices.length < walls.length + 1) {
      ...
      if (visited.has(currentKey) && orderedVertices.length > 2) break;
      ...
      for (const neighbor of currentData.connections) {
        ...
        const isSameAsPrevious = previousPoint && this.pointsEqual(neighbor, previousPoint);
        
        if (!isSameAsPrevious && (!visited.has(neighborKey) || orderedVertices.length >= walls.length)) {
          nextPoint = neighbor;
          break;
        }
      }
  ...
  static ensureCounterClockwise(vertices: { x: number; z: number }[]): { x: number; z: number }[] {
  ...
  static findWallIntersections(walls: Wall[]): { point: { x: number; z: number }; walls: Wall[] }[] {
    ...
    for (const [_, data] of graph.entries()) {
      if (data.connections.length > 2) {
        const connectedWalls = walls.filter(wall => 
          this.pointsEqual(wall.start, data.point) || this.pointsEqual(wall.end, data.point)
        );
  ...
  static generateConnectedWalls(vertices: { x: number; z: number }[], height: number = 2.5, thickness: number = 0.2): Wall[] {
    ...
    for (let i = 0; i < vertices.length; i++) {
      ...
      if (this.distance(start, end) < 0.1) continue;
  ...
  static getRoomStatistics(walls: Wall[]): {
    ...
    return {
      ...
      averageHeight: walls.length > 0 ? walls.reduce((sum, w) => sum + w.height, 0) / walls.length : 0,
      vertices,
      intersections
    };
  }
...
export const useAdvancedRoom = (initialWalls: Wall[] = []) => {
  ...
  const roomStats = useMemo(() => {
    return RoomGeometry.getRoomStatistics(walls);
  }, [walls]);
  ...
  const generateRectangularRoom = useCallback((width: number, height: number, wallHeight: number = 2.5, wallThickness: number = 0.2) => {
    const vertices = [
    ...
    const newWalls = RoomGeometry.generateConnectedWalls(vertices, wallHeight, wallThickness);
    setWalls(newWalls);
  }, []);
  
  const generateLShapedRoom = useCallback((width: number, height: number, cutWidth: number, cutHeight: number, wallHeight: number = 2.5, wallThickness: number = 0.2) => {
